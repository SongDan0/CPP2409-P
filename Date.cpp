...
void Date::AddYear() {
    date[++year_index] = new Schedule*[MAX_MONTHS];          // 다음 연도에 대한 월 배열 생성
    for (int i = 0; i < MAX_MONTHS; i++)                     // 다음 연도의 월에 대한 일 배열 생성
        date[year_index][i] = new Schedule[MAX_DAYS];  
}
...
// 데이터 파일 저장
void Date::SaveToFile(string filename) {
            ...
            for (int m = 0; m < MAX_MONTHS; ++m) {
                for (int d = 0; d < MAX_DAYS; ++d) {
                    ...
                    if (!date[y][m][d].EmptySchedule()) {
                        ...
                        for (const string& schedule : date[y][m][d].GetScheduleVector()) {          
...
// 파일에서 데이터를 불러오기
void Date::LoadFromFile(string filename) {
